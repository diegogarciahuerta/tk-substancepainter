...
class Client(QtCore.QObject):
    ...
    def on_state_changed(self, state):
        ...
        attempts = 0
        while attempts < self.max_attemps and self.client.state() not in (QAbstractSocket.SocketState.ConnectedState,):
            attempts += 1
    ...
    def on_text_message_received(self, message):
        ...
        if jsonData.has_key('result'):
            ...
            if message_id in self.callbacks:
                self.log_debug("client: requesting callback result for message: %s" % message_id)            
    ...
    def send_text_message(self, command, message_id=None, callback=None, **kwargs):
        if self.client.state() in (QAbstractSocket.SocketState.ClosingState,
                                   QAbstractSocket.SocketState.UnconnectedState):
            self.log_debug("client: is not connected!, ignoring message: %s" % message_id)            
            return
    ...
    def on_pong(self, elapsedTime, payload):
        self.log_debug("client: onPong - time: {} ; payload: {}".format(elapsedTime, payload))
...
class EngineClient(Client):
    ...
    def info(self):
        result = self.send_and_receive("INFO")
        return result
...
# def get_application_version(connection):
#     version = connection.send_and_receive("GET_VERSION")
#     connection.log_debug("version: %s (%s)" % (version, type(version)))
#     painter_version = version['painter']
#     connection.log_debug("painter_version: %s" % painter_version)
#     return painter_version


# def get_current_project_path(connection):
#     path = connection.send_and_receive("GET_CURRENT_PROJECT_PATH")
#     connection.log_debug("CURRENT_PROJECT_PATH: %s (%s)" % (path, type(path)))
#     return path

# def need_saving(connection):
#     result = connection.send_and_receive("NEEDS_SAVING", path=path)
#     return result


# def open_project(connection, path):
#     path = connection.send_and_receive("GET_CURRENT_PROJECT_PATH", path=path)


# def save_project_as(connection, path):
#     success = connection.send_and_receive("SAVE_PROJECT_AS", path=path)
#     return success


# def save_project(connection):
#     success = connection.send_and_receive("SAVE_PROJECT")
#     return success


# def close_project(connection):
#     success = connection.send_and_receive("CLOSE_PROJECT")
#     return success


# def broadcast_event(connection, event_name):
#     connection.send_text_message(event_name)
